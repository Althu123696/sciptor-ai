...
function initDB(): DB {
  if (!fs.existsSync(DB_PATH)) {
    const initial: DB = {
      users: [
        {
          ...
          passwordHash: 'admin123', // Standard initial password for admin, can be registered on signup
          role: 'admin',
          usedSeconds: 0,
      ...
      settings: {
        monthlyLimitSeconds: 4500 // 1 hour 15 minutes default
      },
      feedbacks: [],
      logs: [
        {
  ...
  try {
    ...
    return {
      users: [{ email: 'althafka944@gmail.com', passwordHash: 'admin123', role: 'admin', usedSeconds: 0, createdAt: new Date().toISOString() }],
...
// 🧠 API Route: AI Operations (Summarize, Action Items, Translate)
app.post('/api/ai-action', async (req, res) => {
  const { segments, task, options } = req.body;
